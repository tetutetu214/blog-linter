...
- 2026-09-21 **held-out の一発テストも外した**。本人が一度も触っていない Claude 執筆の記事（`content/posts/coop-camera-video/index.mdx`、未公開のため評価データに含まれない）を投げたところ、3 問すべてで 0.21〜0.49、つまり「LLM が書いた文章ではない」と判定した。100% LLM が書いた文章に対する判定としては明確な誤り
- 2026-09-21 **評価データ側は物差しとして機能したので採用**。「Jev はこの用途に使えない」という判定を数値で出せた。前回の文単位版のように表層で当たってしまう欠陥は無い（表層最良 0.755 に対し多数派ベースライン 0.604）
