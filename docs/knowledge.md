...
- 2026-09-21 **節レコードの除外規則**: 可視 200 文字未満、および初稿・公開版で正規化類似度 0.98 以上（本人が直していない節は、どちらのラベルでも同じ文章になり評価にならない）。実データでの版間最大類似度は 0.562（aws-lambda-durable-functions の「はじめに」、284 字 vs 335 字）で除外は 0 件。ただしこれは「本人が全節を書き直した」証拠ではなく、節の刻み方が版間でずれていることでも説明がつく（上の「節は版間で対応していない」を参照）。なお difflib の SequenceMatcher は既定の autojunk=True だと日本語の長文で頻出文字を junk 扱いして類似度を低く出すため（同じ節で 0.501 と 0.562）、実装では autojunk=False を明示している
